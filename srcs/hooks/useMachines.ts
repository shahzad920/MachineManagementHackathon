...
export const useMachines = () => {
    ...
    //@ts-ignore
    const MachineReducer = useSelector(({ MachineReducer }: { MachineReducer: CategoryInterface[] }) => MachineReducer);


    return ({
        actions: {
            AddCategory: () => { dispatch({ type: MachineActions.AddCategory }) },
            ...
            EditCategoryField:
                (payload: { id: string; fieldId: string; name: string }) =>
                    dispatch({ type: MachineActions.EditCategoryField, payload }),
            DeleteCategoryField:
                (payload: { id: string; fieldId: string }) =>
                    dispatch({ type: MachineActions.DeleteCategoryField, payload }),

            AddMachine:
                (payload: { id: string }) =>
                    dispatch({ type: MachineActions.AddCategory, payload }),
            EditMachine:
                (payload: { id: string; machineId: string; name: string }) =>
                    dispatch({ type: MachineActions.EditCategory, payload }),
            DeleteMachine:
                (payload: { id: string; machineId: string }) =>
                    dispatch({ type: MachineActions.EditCategory, payload }),
